...
def solve_radial(
    ...
    **options
    ...
    while True:
        ...
        for l, iz in enumerate(zero_crossings):
            ...
            while True:
                ...
                try:
                    ...
                    if not binfo.converged:
                        logger.info(
                            f'Binary search was not converged with beta={beta},\n{binfo}')
                    ...
                    logger.warning(
                        f'Retrying by changing r_max to {r_max/radius:.2f}a')
            ...
            if m == 0:
                ...
                modes.profiles[-1] = modes.profiles[-1] / \
                    np.sqrt(np.sum(np.abs(modes.profiles[-1])**2))
                ...
                for s, phi_func in zip([-1, 1], phi_funcs):
                    ...
                    modes.profiles.append(
                        f(indexProfile.R).ravel() * phi_func(m*indexProfile.TH.ravel()))
                    modes.profiles[-1] = modes.profiles[-1] / \
                        np.sqrt(np.sum(np.abs(modes.profiles[-1])**2))
                    # is the mode a propagative one?
                    modes.propag.append(True)
    ...
    # sort the modes
    modes.sort()
    logger.info("Solver found %g modes is %0.2f seconds." %
