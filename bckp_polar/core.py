...
class propagationModeSolver():
    ...
    def solve(self, mode = 'default', curvature = None, storeData = True, **options):
        ...
        For an arbitrary index profile, it find the solution of the eigenvalue problem of the scalar wave equation in a discretized space [1].
        '''
    ...
    def solve_eig(self,curvature,nmodesMax=6,boundary = 'close', propag_only = True):#,storeData = True,curvature = None):
        ...
        logger.info(f"Grid type: {self.indexProfile.grid_type}")
        
        ## Construction of the operator 
        
        if self.indexProfile.grid_type == 'cartesian':
    ...
    #            curv_mat = sparse.diags(1./(1.+2*xi*self.indexProfile.X.flatten()/curvature), dtype = np.complex128)
        ...
        elif self.indexProfile.grid_type == 'polar':
            logger.info('Matrix in the polar coordinates.') 
            dr = self.indexProfile.dr
            dth = self.indexProfile.dth
            Rsq = self.indexProfile.R.flatten()**2
            nr = self.indexProfile.n_r
            nth = self.indexProfile.n_theta
            rsq =  Rsq[:nr]
            
            diags.append(-2./dr**2-1./Rsq*(1./2.+2./dth**2)+k0**2*self.indexProfile.n.flatten()**2)
            
            # radial component of the Laplacian
            diags.append(([1./dr**2]*(nr-1)+[0.])*(nth-1)+[1./dr**2]*(nr-1))
            diags.append(([1./dr**2]*(nr-1)+[0.])*(nth-1)+[1./dr**2]*(nr-1))
            
            # azimutal component of the Laplacian
            diags.append(np.tile(1./dth**2*1./rsq,nth-1).flatten())
            diags.append(np.tile(1./dth**2*1./rsq,nth-1).flatten())
            
            diags.append(1./dth**2*1./rsq)
            diags.append(1./dth**2*1./rsq)
            
            dd = sparse.diags(Rsq,dtype=np.complex128)#            
#            
#            diags.append(-2.*Rsq/dr**2-(1./2.+2./dth**2)+Rsq*k0**2*self.indexProfile.n.flatten()**2)
##            print(diags[-1].shape)
#            
#            # radial component of the Laplacian
##            diags.append(([1./dr**2]*(nr-1)+[0.])*(nth-1)+[rsq/dr**2]*(nr-1))
#            diags.append(np.tile(np.concatenate([rsq[:-1]/dr**2,[0]]),nth).flatten()[:-1])
#            diags.append(np.tile(np.concatenate([rsq[:-1]/dr**2,[0]]),nth).flatten()[:-1])
##            print(diags[-1].shape)
##            print(len(([1./dr**2]*(nr-1)+[0.])*(nth-1)+[1./dr**2]*(nr-1)))
#            
#            # azimutal component of the Laplacian
#            diags.append([1./dth**2]*nr*(nth-1))
#            diags.append([1./dth**2]*nr*(nth-1))
##            diags.append(np.tile(1./dth**2,nth-1).flatten())
##            print(diags[-1].shape)
#            
#            diags.append([1./dth**2]*nr)
#            diags.append([1./dth**2]*nr)
#            print(diags[-1].shape)
#            print('--'*20)
#            print(len([1./dth**2*1./rsq]*(nth-1)))
            
            offsets = [0,-1,1,-nr,nr,(nth-1)*nr,-(nth-1)*nr]
        ...
        if self.indexProfile.grid_type == 'polar':
            H = dd.dot(H)
        

#        plt.figure()
#        plt.imshow(np.real(H.todense()))
...
#            if curvature is not None or (betasq > beta_min**2 and betasq < beta_max**2):
             if (betasq > beta_min**2 and betasq < beta_max**2) or not propag_only:
                ...
                modes.number+=1
                if self.indexProfile.grid_type == 'polar':
                    modes.profiles.append(res[1][:,i]*1./np.sqrt(Rsq))
                else:   
